...
module Lims::Api
  describe Context do
    ...
    subject { described_class.new(mock("Store"), mock("MessageBus"), mock("application_id"), url_generator, '', mock("user"), mock("pipeline_id")) }
    ...
    subject { described_class.new(mock("Store"), mock("MessageBus"), mock("application_id"), url_generator, 'test;test_mime=true', mock("user"), mock("pipeline_id")) }
    ...
    context "#uuid" do
      context "with an valid uuid" do
        ...
        subject { described_class.new(store, message_bus, application_id, url_generator, '', user, pipeline_id) }
    ...
    context "#for_root" do
      subject { described_class.new(mock(:store), mock(:message_bus), mock(:application_id), url_generator, '', mock("user"), mock("pipeline_id")).for_root }
      it "is a resource" do
